package _10_B_RreesAndRed_BlackTrees.Lab;
...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    private Node rotateLeft(Node node) {
        ...
        newRoot.childrenCount = node.childrenCount;
    ...
    private void flipColors(Node node) {
        ...
        if (node.left != null) {
            node.left.setColor(this.BLACK);
        }

        if (node.right != null) {
            node.right.setColor(this.BLACK);
        }
    }
