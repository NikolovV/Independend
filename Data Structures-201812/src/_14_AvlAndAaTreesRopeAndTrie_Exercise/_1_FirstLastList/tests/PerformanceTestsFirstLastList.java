...
public class PerformanceTestsFirstLastList {
    ...
    public void testPerformance_First() {
        ...
        for (int i = 0; i < 500; i++) {
            this.addProducts(1);
            Product[] returnedProducts = TestUtilities.iterableToStream(this.products.first(i)).toArray(Product[]::new);
    ...
    public void testPerformance_Last() {
        ...
        for (int i = 0; i < 500; i++) {
            this.addProducts(1);
            Product[] returnedProducts = TestUtilities.iterableToStream(this.products.last(i)).toArray(Product[]::new);
            assertEquals(i, returnedProducts.length);
        }
    }
    ...
    public void testPerformance_Min() {
        ...
        for (int i = 0; i < 230; i++) {
            this.addProducts(1);
            Product[] returnedProducts = TestUtilities.iterableToStream(this.products.min(i)).toArray(Product[]::new);
            assertEquals(i, returnedProducts.length);
        }
    }
    ...
    public void testPerformance_Max() {
        ...
        for (int i = 0; i < 230; i++) {
            this.addProducts(1);
            Product[] returnedProducts = TestUtilities.iterableToStream(this.products.max(i)).toArray(Product[]::new);
            assertEquals(i, returnedProducts.length);
        }
    }
    ...
    public void testPerformance_RemoveAll() {
        ...
        while (this.products.getCount() > 0) {
            this.addProducts(1);
            Product first = TestUtilities.iterableToStream(this.products.first(1)).findFirst().get();
    ...
    private void addProducts(int count) {
        for (int i = 0; i < count; i++) {
            this.addCounter++;
            this.products.add(new Product(this.addCounter % 1000, "Product" + this.addCounter));
        }
    }
}
